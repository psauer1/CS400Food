...
public class BPTree<K extends Comparable<K>, V> implements BPTreeADT<K, V> {
	 ...
	 */
	public BPTreeNA(int branchingFactor) {
		...
		while (!queue.isEmpty()) {
			...
			while (!queue.isEmpty()) {
				...
				while (it.hasNext()) {
					...
					if (it.hasNext())
						sb.append(", ");
					if (node instanceof BPTreeNA.InternalNode)
	...
	private class InternalNode extends Node {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see BPTreeNA.Node#getFirstLeafKey()
		 */
		K getFirstLeafKey() {
			return keys.get(0);
		}
		 ...
		 * (non-Javadoc)
		 * 
		 * @see BPTreeNA.Node#insert(java.lang.Comparable, java.lang.Object)
		 */
		 ...
		 * (non-Javadoc)
		 * 
		 * @see BPTreeNA.Node#rangeSearch(java.lang.Comparable, java.lang.String)
		 */
	...
	private class LeafNode extends Node {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see BPTreeNA.Node#insert(Comparable, Object)
		 */
		void insert(K key, V value) {
			int valueIndex = 0;
			for (int i = 0; i < keys.size(); i++) {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see BPTreeNA.Node#rangeSearch(Comparable, String)
		 */
		List<V> rangeSearch(K key, String comparator) {
	...
	public static void main(String[] args) {
		...
		BPTreeNA<Double, Double> bpTree = new BPTreeNA<>(3);
