...
public class Main extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		sortSection.sortFood.setOnMouseClicked((event) -> {
			...
			if (sortSection.nameBox.isSelected()) { // if name box is checked
				try {
					...
					for(FoodItem nameFiltered:all.filterByName(sortSection.lowname.getText())) { // if the search item contains
			...
			analyzeSection.foodList.setItems(FXCollections.observableArrayList(nameList)); // displays names from foodList
		});
		...
		addSection.addFood.setOnMouseClicked((event) -> {
			...
			try {
				...
				FoodItem newFood = new FoodItem(secretKey,addSection.entername.getText(), Double.valueOf(addSection.entercalories.getText()),
						Double.valueOf(addSection.enterfats.getText()), Double.valueOf(addSection.enterprotein.getText()),
						Double.valueOf(addSection.entercarbs.getText()), Double.valueOf(addSection.enterfiber.getText()));
		...
		analyzeSection.addToMeal.setOnMouseClicked((event) -> {
			FoodItem addItem = all.getFood(analyzeSection.foodList.getSelectionModel().getSelectedItem());	// copy highlighted items from food list to the meal list
			menu.get().addFoodItem(addItem);
			analyzeSection.mealList.setItems(FXCollections.observableArrayList(menu.get().getNames())); // displays names from foodList
		});
		...
		analyzeSection.removeFromMeal.setOnMouseClicked((event) -> {
			menu.get().removeFoodItem(analyzeSection.mealList.getSelectionModel().getSelectedItem());	// remove highlighted items from meal list
			analyzeSection.mealList.setItems(FXCollections.observableArrayList(menu.get().getNames())); // displays names from foodList
		});
		...
		addSection.importFood.setOnMouseClicked((event) -> {
			...
			analyzeSection.foodList.setItems(FXCollections.observableArrayList(all.getNames()));	// display imported items in the food list
		});
