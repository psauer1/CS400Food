...
public class FoodData implements FoodDataADT<FoodItem> {
	...
	private HashMap<String, TreeMap<Double, ArrayList<FoodItem>>> indexes = new HashMap<>(); // Map of nutrients and
																								// their corresponding
																								// index
	 ...
	 * Public constructor
	 * 
	 * Generates an empty data structure to store food
	 */
	public FoodData() {
	...
	public List<FoodItem> filterByNutrients(List<String> rules) {
		...
		for (String rule : rules) { // iterate over the list of sorting rules
			String[] r = rule.split(" ");
			if (r[1].equals("==")) { // gets all of the nutrients that are equal to ruler
				subList = subList.getFoodRange(r[0], Double.valueOf(r[2]), Double.valueOf(r[2]));
			} else if (r[1].equals(">=")) { // gets all of the nutrients that are greater than or equal to rule
				subList = subList.getFoodRange(r[0], Double.valueOf(r[2]), (double) 100000);
			} else if (r[1].equals("<=")) { // gets all of the nutrients that are less than or equal to rule
	...
	/**
	 * Gets the food item that matches the parameter name, throws an exception if
	 * there is not match
	 * 
	 * @param name
	 * @return FoodItem
	...
	public FoodItem getFood(String name) {
		for (FoodItem food : foodItemList) { // searches for food with the same name
			if (food.getName().equals(name))
				return food;
		}
	...
	/**
	 * Creates an input for a file to be exported to a .csv file with the proper
	 * format for reading in a file
	 * 
	 * @param filename
	 */
	public void saveFoodItems(String filename) {
		File newCSV = new File(filename); // creates file to be save the food items to
		PrintWriter pw = null;
		try {
	...
	private FoodData getFoodRange(String key, Double low, Double high) throws IllegalArgumentException {
		Collection<ArrayList<FoodItem>> searchList = indexes.get(key).subMap(low, high).values(); // gets values from
																									// input range
	...
	/**
	 * Private helper Inserts item all of the tree maps within the hash map
	 *
	 * @param item
	 */
